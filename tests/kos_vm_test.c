                                  ...
                                  const KOS_FUNCTION_OPTS *opts)
{
    ...
    func->handler.bytecode = kos_alloc_bytecode(ctx, &bytecode[offs], bytecode_size - offs, KOS_NULL, 0);
    if (IS_BAD_PTR(func->handler.bytecode))
        return KOS_BADPTR;
...
static KOS_OBJ_ID create_class(KOS_CONTEXT              ctx,
                               ...
                               const KOS_FUNCTION_OPTS *opts)
{
    return create_func_obj(ctx, CREATE_CLASS, bytecode, bytecode_size, offs, opts);
}
                           ...
                           unsigned       num_constants)
{
    ...
    if ( ! error) {
        ...
        func_obj = create_func_obj(ctx, CREATE_FUNC, bytecode, bytecode_size, 0, &opts);
...
int main(void)
{
    ...
    {
        const uint8_t code[] = {
            INSTR_NEW_ARRAY8,  0, 5,
            INSTR_LOAD_INT8,   1, 3,
            INSTR_LOAD_INT8,   2, 10,
            INSTR_SET,         0, 1, 2,
            INSTR_GET_ELEM8,   1, 0, (uint8_t)(int8_t)-2,
            INSTR_RETURN,      1
        };
    ...
    /************************************************************************/
    /* SET.ELEM8 - index out of range */
    {
        static const char prop1[]  = "prop1";
        KOS_OBJ_ID        str_prop = KOS_new_const_ascii_cstring(ctx, prop1);
        const uint8_t code[] = {
            INSTR_NEW_ARRAY8,   0, 1,
            INSTR_LOAD_CONST,   1, 0,/*"prop1"*/
            INSTR_SET_ELEM8,    0, 1, 1,
            INSTR_RETURN,       0
        };
    ...
    /************************************************************************/
    /* SET.ELEM8 - invalid index type for array */
    {
        static const char prop1[]  = "prop1";
        KOS_OBJ_ID        str_prop = KOS_new_const_ascii_cstring(ctx, prop1);
        const uint8_t code[] = {
            INSTR_LOAD_CONST,   0, 0,/*"prop1"*/
            INSTR_SET_ELEM8,    0, 0, 0,
            INSTR_RETURN,       0
        };
    ...
    /* PUSH */
    {
        ...
        KOS_OBJ_ID        val;
        const uint8_t code[] = {
            INSTR_NEW_ARRAY8,   0, 0,
            INSTR_NEW_ARRAY8,   1, 1,
            INSTR_LOAD_INT8,    2, 10,
            INSTR_SET_ELEM8,    1, 0, 2,
            INSTR_PUSH,         1, 0,
            INSTR_PUSH,         1, 1,
            INSTR_LOAD_CONST,   2, 0/*"prop5"*/,
            INSTR_PUSH,         1, 2,
            INSTR_RETURN,       1
        };
    ...
    /* LOAD.CONST (function), CALL */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(2, 1);
        KOS_OBJ_ID        func = create_func(ctx, &code[0], sizeof(code), 29, &opts);
    ...
    /* CALL.FUN - too many args */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(1, 0);
        KOS_OBJ_ID        func = create_func(ctx, &code[0], sizeof(code), 13, &opts);
    ...
    /* LOAD.CONST (function), CALL.N - reuse function body twice */
    {
        ...
        opts.this_reg = 1;
        constants[0] = create_func(ctx, &code[0], sizeof(code), 23, &opts);

        opts = create_func_opts(2, 0);
        opts.this_reg = 0;
        constants[1] = create_func(ctx, &code[0], sizeof(code), 30, &opts);
    ...
    /* GET.PROTO */
    {
        ...
        opts = create_func_opts(1, 0);
        constants[0] = create_class(ctx, &code[0], sizeof(code), 0, &opts);
    ...
    /* LOAD.CONST (generator), CALL - instantiate generator */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(1, 0);
        KOS_OBJ_ID        func = create_gen(ctx, &code[0], sizeof(code), 3, &opts);
    ...
    /* LOAD.CONST (generator), CALL.N/FUN */
    {
        ...
        opts = create_func_opts(1, 0);
        opts.this_reg = 0;
        constants[1] = create_gen(ctx, &code[0], sizeof(code), 19, &opts);
    ...
    /* LOAD.CONST (generator), YIELD */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(1, 0);
        KOS_OBJ_ID        func = create_gen(ctx, &code[0], sizeof(code), 15, &opts);
    ...
    /* LOAD.CONST (generator), CALL - not enough args */
    {
        ...
        KOS_OBJ_ID        func = create_gen(ctx, &code[0], sizeof(code), 16, &opts);
    ...
    /* LOAD.CONST (generator), CALL - one arg passed to generator in "READY" state */
    {
        ...
        opts.this_reg = 1;
        func = create_gen(ctx, &code[0], sizeof(code), 19, &opts);
    ...
    /* YIELD - pass data to generator through yield */
    {
        ...
        opts.this_reg = 0;
        func = create_gen(ctx, &code[0], sizeof(code), 60, &opts);
    ...
    /* NEXT - call beyond the end of generator */
    {
        ...
        opts.this_reg = 0;
        func = create_gen(ctx, &code[0], sizeof(code), 16, &opts);
    ...
    /* YIELD - yield not supported in a regular function */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(1, 0);
        KOS_OBJ_ID        func = create_func(ctx, &code[0], sizeof(code), 14, &opts);
    ...
    /* TAIL.CALL.N */
    {
        ...
        opts.this_reg = 2;
        func = create_func(ctx, &code[0], sizeof(code), 21, &opts);
    ...
    /* TAIL.CALL.FUN */
    {
        ...
        KOS_FUNCTION_OPTS opts = create_func_opts(3, 2);
        KOS_OBJ_ID        func = create_func(ctx, &code[0], sizeof(code), 17, &opts);
    ...
    /* BIND, BIND.SELF */
    {
        const uint8_t code[] = {
            ...
            INSTR_GET_ELEM8,   0, 1, 0,
            INSTR_LOAD_INT8,   1, 4,
            INSTR_ADD,         0, 0, 1,
            INSTR_SET_ELEM8,   2, 1, 0,
            INSTR_LOAD_VOID,   0,
            INSTR_RETURN,      0
        };
        ...
        opts.bind_reg     = 1;
        opts.num_binds    = 1;
        constants[0] = create_func(ctx, &code[0], sizeof(code), 22, &opts);
    ...
    /* BIND, BIND.SELF */
    {
        const uint8_t code[]   = {
            ...
            INSTR_SET_ELEM8,   3, 0, 4,
            INSTR_LOAD_CONST,  4, 0,
            ...
            INSTR_GET_ELEM8,   0, 2, 0,
            INSTR_GET_ELEM8,   3, 1, 4,
            INSTR_ADD,         0, 0, 3,
            INSTR_SET_ELEM8,   1, 4, 0,
            INSTR_LOAD_INT8,   0, 0,
            INSTR_YIELD,       0, 0
        };
        ...
        opts.bind_reg  = 1;
        opts.num_binds = 2;
        constants[1] = create_gen(ctx, &code[0], sizeof(code), 40, &opts);
    ...
    /* BIND, BIND.SELF - independent variables */
    {
        const uint8_t code[] = {
            ...
            INSTR_LOAD_CONST,  5, 0,                 /* Read by level 2 */
            INSTR_NEW_ARRAY8,  1, 2,
            INSTR_LOAD_INT8,   0, 9,
            INSTR_SET_ELEM8,   1, 0, 0,
            INSTR_LOAD_CONST,  0, 1,
            ...
            INSTR_NEW_ARRAY8,  1, 3,
            INSTR_SET_ELEM8,   1, 2, 0,       /* 0x100 */
            INSTR_LOAD_INT8,   0, 5,
            INSTR_CALL,        2, 3, 0, 1,
            INSTR_RETURN,      2,
             ...
             * 4 - global regs
             * 5 - global integer */
            INSTR_GET_ELEM8,   6, 4, (uint8_t)(int8_t)-2,      /* 3 */
            INSTR_LOAD_CONST,  0, 3,
            ...
            INSTR_BIND,        6, 3, 2,
            INSTR_BIND,        6, 4, 3,
            INSTR_SET_ELEM8,   4, (uint8_t)(int8_t)-3, 6,
            INSTR_RETURN,      3,
            ...
            INSTR_GET_ELEM8,   2, 102, (uint8_t)(int8_t)-1, /* 0x40000 */
            INSTR_GET_ELEM8,   4, 104, (uint8_t)(int8_t)-1, /* 0x1000 */
        ...
        opts.bind_reg     = 4;
        opts.num_binds    = 2;
        constants[4] = create_func(ctx, &code[0], sizeof(code),  68, &opts);
        ...
        opts.bind_reg  = 102;
        opts.num_binds = 5;
        constants[5] = create_func(ctx, &code[0], sizeof(code), 111, &opts);
    ...
    /* BIND.DEFAULTS - all default values */
    {
        ...
        opts.num_def_args = 3;
        opts.closure_size = 3;
        constants[0] = create_func(ctx, &code[0], sizeof(code), 34, &opts);

        opts = create_func_opts(3, 0);
        opts.bind_reg  = 1;
        opts.num_binds = 1;
        constants[1] = create_func(ctx, &code[0], sizeof(code), 44, &opts);
    ...
    /* BIND.DEFAULTS - some default values */
    {
        ...
        opts.num_def_args = 3;
        opts.closure_size = 5;
        constants[0] = create_func(ctx, &code[0], sizeof(code), 40, &opts);

        opts = create_func_opts(3, 0);
        opts.bind_reg  = 1;
        opts.num_binds = 1;
        constants[1] = create_func(ctx, &code[0], sizeof(code), 52, &opts);
