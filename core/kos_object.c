...
static KOS_OBJ_ID new_builtin_dynamic_pro2(KOS_CONTEXT          ctx,
                                           KOS_OBJ_ID           module_obj,
                                           KOS_OBJ_ID           name_obj,
                                           ...
                                           KOS_FUNCTION_HANDLE2 setter)
{
    ...
    if ( ! IS_BAD_PTR(dyn_prop.o)) {
        ...
        if ( ! IS_BAD_PTR(func_obj)) {
            OBJPTR(FUNCTION, func_obj)->module          = module_obj;
            OBJPTR(FUNCTION, func_obj)->opts.min_args   = 0;
            OBJPTR(FUNCTION, func_obj)->handle2.handler = getter;
            OBJPTR(FUNCTION, func_obj)->name            = name.o;
            OBJPTR(DYNAMIC_PROP, dyn_prop.o)->getter    = func_obj;
        }
        else
    ...
    if ( ! IS_BAD_PTR(dyn_prop.o)) {
        ...
        if (setter) {
            ...
            if ( ! IS_BAD_PTR(func_obj)) {
                OBJPTR(FUNCTION, func_obj)->module          = module_obj;
                OBJPTR(FUNCTION, func_obj)->opts.min_args   = 0;
                OBJPTR(FUNCTION, func_obj)->handle2.handler = setter;
                OBJPTR(FUNCTION, func_obj)->name            = name.o;
                OBJPTR(DYNAMIC_PROP, dyn_prop.o)->setter    = func_obj;
            }
            else
                                     ...
                                     KOS_FUNCTION_HANDLE2 setter)
{
    ...
    dyn_prop = new_builtin_dynamic_pro2(ctx, module_obj, prop.o, getter, setter);
                               ...
                               enum KOS_DEPTH_E depth)
{
    ...
    KOS_init_locals(ctx, &obj, &walk, kos_end_locals);
    obj.o = obj_id;
                    ...
                    KOS_OBJ_ID  iterator_id)
{
    ...
    KOS_init_locals(ctx, &walk, &table, &returned_keys, &key, kos_end_locals);
    walk.o = iterator_id;
