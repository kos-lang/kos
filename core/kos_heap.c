                              ...
                              KOS_OBJ_ID        obj_id)
{
    ...
    switch (READ_OBJ_TYPE(obj_id)) {
        ...
        case OBJ_FUNCTION:
            ...
            if ( ! KOS_is_native_function(obj_id))
...
static int mark_roots(KOS_CONTEXT ctx, KOS_MARK_CONTEXT *mark_ctx)
{
    ...
    TRY(mark_object_black(mark_ctx, inst->prototypes.generator_close_proto));
...
static void update_child_ptrs(KOS_OBJ_HEADER *hdr)
{
    switch (kos_get_object_type(*hdr)) {
        ...
        case OBJ_FUNCTION:
            if ( ! KOS_is_native_function((KOS_OBJ_ID)((intptr_t)hdr + 1)))
...
static void update_after_evacuation(KOS_CONTEXT ctx)
{
    ...
    update_child_ptr(&inst->prototypes.generator_close_proto);
